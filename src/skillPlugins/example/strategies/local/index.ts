...
export = {
    ...
    */
    async online(options: StrategyOnlineOptions): Promise<OnlineResult<void>> {
        try {
            ...
            console.log("example plugin online with options:", options);
            state.online = true;
    ...
    */
    async offline(): Promise<OfflineResult<void>> {
        try {
            console.log("example plugin offline");
            state.online = false;
            return { ok: true, value: undefined };
        }catch (error) {
    ...
    async restart(options: StrategyRestartOptions): Promise<RestartResult<void>> {
        try {
            ...
            console.log("example plugin restart with options:", options);
    ...
    async send(options: SendOptions) : Promise<SendResult<void>>{
        try {
            ...
            console.log("example plugin send with options:", options);
